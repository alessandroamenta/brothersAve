...
class FoodieInboxVC: UIViewController {
    ...
    func setupView(){
        ...
        Firestore.firestore().collection("users").document(Auth.auth().currentUser?.uid ?? "").getDocument { snapShot, error in
            if error != nil {  }
                ...
                self.createDialog(id: NSNumber(value: self.friend!.QBID))
                self.MessageTableView.reloadData()
            }
        }
        MessageTableView.register(UINib(nibName: "FoodieMessagesCell", bundle: nil), forCellReuseIdentifier: "FoodieMessagesCell")
                ...
                if messages.count > 0 {
                    for message in messages {
                        if message.attachments?.first != nil {
                            if message.senderID == self.friend?.QBID {
        ...
        QBRequest.tUploadFile(uploadData, fileName: "myImage", contentType: "image/png", isPublic: true, successBlock: { (response, uploadedBlob) in
            ...
            self.chatDialog.send(message, completionBlock:
                ...
                if let error = error {
                    ...
                    self.sendSingleNotification(title: "Food App", message: "Message from \(appCredentials.name ?? "Someone"): \(self.messageText.text!)", receiverToken: self.friend!.fcmToken)
                    self.updateContactList(chefId: self.friend?.id ?? "")
                    DispatchQueue.main.async {
                        RappleActivityIndicatorView.stopAnimation()
                        self.MessageTableView.reloadData()
                        let indexPath = IndexPath(row: self.messages.count - 1, section: 0)
    ...
    func sendPushNotificxation(){
        ...
        let receiverID = self.friend?.QBID
    ...
    @IBAction func sendBtnAction(_ sender:UIButton){
        ...
        if messageText.text == "" && selectedImagesFromGallary.count <= 0 { presentAlert(withTitle: "Error", message: "Please enter message text or select an Image") }
        else {
...
//MARK: - TablewView delegate and Data Source methods...
extension FoodieInboxVC : UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = MessageTableView.dequeueReusableCell(withIdentifier: "FoodieMessagesCell", for: indexPath) as! FoodieMessagesCell
        ...
        if message.senderId == self.current_user_QBID {
            ...
            cell.leftUserImage.sd_setImage(with: URL(string: (friend?.profilePic)!))
        }
...
extension FoodieInboxVC: QBChatDelegate {
...
//MARK: - Network Layers...
extension FoodieInboxVC {
    ///updateContactList...
    func updateContactList(chefId : String){
        
        FoodieService.instance.updateContactList(userId: chefId) { resultent in
            switch resultent {
    ...
    func sendSingleNotification(title: String, message: String, receiverToken: String){
        constants.servicesManager.sendSingleleNotification(title: title, message: message, receiverToken: receiverToken, completion: { resultent in
            ...
            case .success(let response):
                //if alertMsg != "" {RappleActivityIndicatorView.stopAnimation();self.presentAlert(withTitle: alertTitle, message: alertMsg)}
