...
//  Food App
//
//  Created by Syed Saood Ul Hasnain on 18/10/2021.
//
...
class DateTimePicker: NSObject, UIPickerViewDelegate, UIPickerViewDataSource {
 ...
 var didSelectDates: ((_ start: Date, _ end: Date, _ selectDate: Date) -> Void)?
  ...
  func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
    ...
    didSelectDates?(startTime, endTime, days[dayIndex])
  }
  ...
  private func getDays(of date: Date) -> [Date] {
    ...
    // adding 30 days to current date
      let oneMonthFromNow = calendar.date(byAdding: .year, value: 1, to: currentDate)
...
extension Date {
    ...
    static func buildTimeRangeString(startDate: Date, endDate: Date,selectDate: Date) -> String {
    ...
    return String(format: "%@ (%@ - %@)",
                  dayFormatter.string(from: selectDate),
